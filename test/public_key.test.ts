...
import get_public_key from "../src/get_public_key";
...
const private_key_negative_y = new Uint8Array([
  253, 156, 168, 92, 14, 7, 242, 47, 88, 234, 12, 42, 46, 235, 100, 3, 182, 58,
  108, 250, 164, 92, 66, 252, 93, 171, 39, 59, 188, 182, 83, 97,
]);
...
it("Public key from private key.", async () => {
  const pub_key = await get_public_key(private_key);
  deepStrictEqual(pub_key, public_key, "Expected public key from private key");
});

it("Range error public key from private key.", async () => {
  const pub_key = new Uint8Array(32).fill(255);
  rejects(async () => get_public_key(pub_key), "Expected throw 2");
});

it("Public key with odd y coordinate 2.", async () => {
  deepStrictEqual(
    public_key_negative_y,
    await get_public_key(private_key_negative_y)
  );
});
