...
it("ECDSA test.", async () => {
  ...
  sign({ private_key, hash }).then(({ r, s }) => {
    ...
    deepStrictEqual(
      s,
      Uint8Array.from([
        111, 249, 137, 86, 106, 238, 64, 221, 142, 155, 69, 155, 220, 248, 115,
        86, 40, 253, 185, 203, 14, 216, 101, 226, 70, 25, 127, 31, 145, 3, 72,
        142,
      ]),
...
it("ECDSA test 2 iterations.", async () => {
...
it("ECDSA test 3 iterations.", async () => {
  const hash = await sha256(
    Uint8Array.from([
...
it("Signature test", async () => {
  ...
  const private_key = Uint8Array.from([
    171, 209, 35, 208, 126, 4, 237, 27, 221, 89, 230, 143, 1, 241, 209, 116, 11,
    185, 87, 240, 82, 81, 52, 97, 243, 122, 3, 93, 4, 243, 118, 38,
  ]);
  ...
  const s = Uint8Array.from([
    117, 74, 55, 83, 76, 159, 217, 167, 33, 55, 244, 36, 161, 89, 133, 89, 167,
    60, 97, 156, 142, 211, 135, 188, 248, 131, 7, 226, 5, 21, 35, 95,
  ]);
...
it("Even more tests", async () => {
  ...
  deepStrictEqual(
    ...
    Uint8Array.from([
      ...
      167, 167, 134, 117, 209, 169, 175, 254, 181, 227, 153, 75, 219, 177, 181,
      223,
    ])
  );
  ...
  deepStrictEqual(
    ...
    Uint8Array.from([
      ...
      243, 106, 151, 122, 221, 154, 230, 31, 199, 33, 26, 40, 34, 108, 10, 176,
    ])
  );
});
