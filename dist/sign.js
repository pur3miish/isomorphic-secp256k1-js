...
async function sign({ private_key, hash, canonical = true, }) {
    ...
    async function deterministically_generate_k(hash, private_key, nonce = 0) {
        ...
        const buff_D = await hmac_sha256(Uint8Array.from([...v, 0, ...private_key, ...msg_digest]), k);
        const buf_E = await hmac_sha256(v, buff_D);
        const buf_F = await hmac_sha256(Uint8Array.from([...buf_E, 1, ...private_key, ...msg_digest]), buff_D);
