...
export function number_to_array(number) {
    ...
    return Uint8Array.from(hn.match(/[a-f0-9A-F]{2}/gmu).map((i) => Number(`0x${i}`)));
}
/**
 * Converts a byte array to a BigInt number.
 *
...
export function array_to_number(array) {
    return BigInt(`0x${array.reduce((acc, i) => (acc += i.toString(16).padStart(2, "0")), "")}`);
}
...
export function double_and_add(G, k, mod, n) {
    ...
    if (k == 1n)
        ...
        return add(double_and_add(G, k - 1n, mod, n), G, mod);
    else
        return double_and_add(dbl(G, mod), k / 2n, mod, n);
}
export function point_from_x(odd, x) {
    const { mod, b, a } = secp256k1;
    odd = BigInt(odd);
    const alpha = get_mod(get_mod(get_mod(BigInt(x) ** 3n, mod) + get_mod(a * x, mod), mod) + b, mod);
